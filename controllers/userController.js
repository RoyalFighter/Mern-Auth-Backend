...
const addFriend = async (req, res) => {
  try {
    ...
    const isAlreadyFriend = user.friends.some(friend => friend.name === friendName);
    ...
    res.status(200).json({ message: "Friend added successfully", id: friend._id, name: friend.name });
  } catch (error) {
    console.error(error);
...
const createChat = async (req, res) => {
  try {
    ...
    const friendIds = req.user.friends.map(friend => friend._id);
  
    const text = req.body.text;
  
    // Assuming you have access to friend names, you can retrieve them here
    const friendNames = req.user.friends.map(friend => friend.name);
  
    // Create a new message object
    const message = new Message({
      senderId: userId,
      receivers: friendIds.map((friendId, index) => ({
        _id: friendId,
        name: friendNames[index], // Use the corresponding friend's name
      })),
      text: text,
    });
    ...
    // Handle any response you want to send
